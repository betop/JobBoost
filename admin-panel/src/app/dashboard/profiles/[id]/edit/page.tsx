...
function EditProfileForm({ profile, id }: { profile: Profile; id: string }) {
  ...
  const {
    ...
    defaultValues: {
      ...
      allowed_languages: profile.allowed_languages ?? "English",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-8">
      ...
      {/* API Mode */}
      <div>
        <h2 className="text-xl font-semibold text-gray-900 mb-1">Generation API</h2>
        <p className="text-sm text-gray-500 mb-4">Use the legacy resume generation API for this profile instead of the current one.</p>
        <Controller
          name="use_legacy_api"
          control={control}
          render={({ field }) => (

...
export default function NewBidderPage() {
  ...
  const createMutation = useMutation({
    mutationFn: bidderService.create,
    onSuccess: () => {
      showToast("Bidder created successfully", "success");
      router.push("/dashboard/bidders");
    },
    onError: () => {
      showToast("Failed to create bidder", "error");
    },
  });

  const onSubmit = (data: BidderFormData) => {
    createMutation.mutate({ ...data, profile_ids: data.profile_ids ?? [] });
  };
  ...
  return (
    <>
      <div className="mb-6 flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold text-gray-900">Create New Bidder</h1>
      </div>

...
query users verb=POST {
  ...
  stack {
    ...
    // If the creator is a regular admin, auto-assign the new bidder to them
    conditional {
      if ($user_type == "bidder" && $auth_user.type == "admin") {
        var $admin_bidder_ids {
          value = $auth_user.assigned_bidder_ids != null ? $auth_user.assigned_bidder_ids : []
        }

        db.patch users {
          field_name = "id"
          field_value = $auth.id
          data = {assigned_bidder_ids: $admin_bidder_ids|push:$b.id}
        } as $_
      }
    }
